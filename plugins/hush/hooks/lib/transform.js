...
//
// The pure helpers stay exported as internal seams for the helper tests.

const { decodeResponse, SHELL_FIELDS } = require("./harness");
const { combineActions, buildRecord, recoveryGap, sizeGap, fieldGap, appendRecord } = require("./transform-manifest");
const { decode: decodeTrailer, hasTrailer } = require("./exit-trailer");
...
// collapseTemplates, called on its own, runs under the defaults, not the
...
// harness's readTurn in the hook and a stub in a test: `turn(transcriptPath)`
// returns `{ promptText, bytes }`. `settings` is what settingsFromEnv
// returns. Nothing here requires session scratch or the transcript reader.
...
// isRangeRead guard in transform() — so that is what it names, with "re-run into a
...
function compressGrep(content, relevanceTokens, fileLabel, decision, sessionId, deps) {
  ...
  const saved = persistGrepMatches(content, sessionId, deps);
  const markerHead =
  ...
  // A view rejected here leaves the persisted copy behind unread — bounded
...
function persistGrepMatches(content, sessionId, deps) {
  return mayPark(content, deps.settings) ? deps.scratch.parkSidecar(sessionId, content) : null;
}

function maybeSidecar(cleaned, relevanceTokens, sessionId, hostMayTruncate, failed, deps) {
  const { settings } = deps;
  ...
  try {
    ...
    const file = deps.scratch.parkSidecar(sessionId, cleaned);
    if (!file) return null;
    const out = view(file);
...
// transform() calls deps.scratch.isSidecar.
...
function compress(text, exitCode, isDump, enumerate, relevanceTokens, scale, sessionId, noSidecar, hostMayTruncate, decision, deps) {
  const { settings } = deps;
  ...
  if (!enumerate && !noSidecar) {
    const side = maybeSidecar(cleaned, relevanceTokens, sessionId, hostMayTruncate, failed, deps);
    if (side !== null) {
      if (decision) {
...
// The exit trailer is hush's own protocol text, and the one view that is
...
// exemption would ship a larger view and still leave the prefix in front
// of the model.
...
// side-channel becomes one manifest record (see transform-manifest.js), the
// record is checked against every product invariant a transform can violate,
// and the view stands only if the record backs it on all of them — recovery
// named for what was removed, no field of a structured response dropped, and
// actually smaller than what it replaces.
//
// A view that fails any of those is a bug in the transform, not something to
// hand the model. There is exactly one fallback and it is the same for all
// three: the view is dropped, the ORIGINAL output stands untouched, and the
...
function deliver(decision, updated, payload, deps) {
  const record = buildRecord({
    ...decision,
    tool: decision.tool || payload.tool_name,
  ...
  if (out !== undefined) {
    ...
    const failure =
      ...
      fail("rejected-field-loss", fieldGap(payload.tool_response, out)) ||
      (mustSanitize(payload.tool_response) ? null : fail("rejected-not-smaller", sizeGap(record)));
    if (failure) {
      record.action = failure.action;
      record.fallback = failure.reason;
      // The view is gone and the original ships whole, so it omitted nothing.
      // Line accounting left describing the dropped view would overstate
      // omission for output that was never trimmed, and the manifest is the
      // trust artifact. Retention only resets when nothing was actually
  ...
  appendRecord(record, deps.scratch);
  ...
  deps.scratch.addSaved(record.session, record.bytesIn, record.bytesOut);
  return { updated: out, record, context: noteFor(out, payload.session_id, deps) };
}
...
// Once per session, the first view that actually leaves a visible [hush
...
// postcompact-rearm.js re-arms it after compaction. Silence never claims it.
function noteFor(updated, sessionId, deps) {
  if (updated === undefined) return undefined;
  const rides = deps.settings.note && hasHushNote(updated) && deps.scratch.claimNote(sessionId);
  return rides ? NOTE_TEXT : undefined;
}

// A watched tool whose response is not a shape hush handles: no view, no
// record, no note.
const SILENT = Object.freeze({ updated: undefined, record: undefined, context: undefined });

// The one interface. `payload` is the raw PostToolUse payload as the host
// sends it. Returns `{ updated, record, context }` — see deliver.
function transform(payload, deps) {
  const { settings } = deps;

  const response = payload.tool_response;
  // One transcript read per fire.
  const turn = deps.turn(payload.transcript_path);
  const enumerate = requestsEnumeration(turn.promptText);
  const relevance = extractRelevanceTokens(turn.promptText);
  const scale = settings.adaptive ? pressureScale(turn.bytes) : 1;
  let updated;
  ...
  if (payload.tool_name === "Read") {
    ...
    const filePath = (payload.tool_input && payload.tool_input.file_path) || (file && file.filePath);
    const sideRead = deps.scratch.isSidecar(filePath);
    ...
    const isRangeRead = !!(payload.tool_input && (payload.tool_input.offset !== undefined || payload.tool_input.limit !== undefined));
    if (file && typeof file.content === "string") {
      ...
      if (!isRangeRead && (isLogPath(filePath) || isGeneratedPath(filePath) || sideRead)) {
        const out = compress(file.content, undefined, true, enumerate, relevance, scale, payload.session_id, sideRead, undefined, decision, deps);
      ...
      return deliver(decision, updated, payload, deps);
    }
    return SILENT;
  }
  ...
  if (payload.tool_name === "Grep") {
    ...
    if (content === null) return deliver({ tool: "Grep", action: "passthrough", bytesIn: 0, linesIn: 0 }, undefined, payload, deps);
    ...
    if (settings.grep && !enumerate && !contextual && content.length >= GREP_MIN_CHARS) {
      const label =
        ...
        undefined;
      out = compressGrep(content, relevance, label, decision, payload.session_id, deps);
    }
    decision.bytesOut = out.length;
  ...
  const isDump = isFileDump(firstLine(payload.tool_input && payload.tool_input.command));
  ...
  if (typeof response === "string") {
    ...
    let out = compress(wrapped ? wrapped.cleanText : response, exitCode ?? undefined, isDump, enumerate, relevance, scale, payload.session_id, undefined, true, decision, deps);
    ...
    for (const field of SHELL_FIELDS) {
      if (typeof next[field] === "string") {
        ...
        const decision = {};
        let out = compress(fieldWrapped ? fieldWrapped.cleanText : next[field], exitCode ?? undefined, isDump, enumerate, relevance, scale, payload.session_id, undefined, true, decision, deps);
