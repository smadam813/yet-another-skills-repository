...
1. **`/grill-with-docs`** sharpens the idea by interview. Start here whenever you work **in a working directory**. It is stateful: it keeps what it learns in `CONTEXT.md` and in ADRs. With no working directory, use **`/grill-me`** instead, covered under Standalone. Both run the same `/grilling` primitive, but `grill-with-docs` leaves a paper trail, so prefer it whenever a repo is there to hold one.
