...
`output-style.md` defines the **Quiet** persona: short sentences, everyday words, no
...
- [`output-style.md`](output-style.md) — the Quiet output style, in Claude Code's
  output-style format.
...
both files from this repo, saves the output style to `~/.claude/output-styles/quiet.md`,
appends the instructions to your project's `CLAUDE.md`, and checks your project's
`.claude/settings.json` for a conflicting output style. It then reports what it did.

```
Set up quiet for me in this project. Do all of it yourself and tell me at the end what changed.

Fetch these two files:

- https://raw.githubusercontent.com/smadam813/yet-another-skills-repository/refs/heads/main/output-styles/quiet/output-style.md
- https://raw.githubusercontent.com/smadam813/yet-another-skills-repository/refs/heads/main/output-styles/quiet/claude-instructions.md
...
1. Copy `output-style.md` to `~/.claude/output-styles/quiet.md`.
...
`"outputStyle": "Quiet"` in `.claude/settings.json` to make it the project default.
