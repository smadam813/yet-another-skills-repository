...
5. A user-invoked engineering skill also goes into `plugins/engineering/skills/ask-builder/SKILL.md`, the router a human reads to find it. Place it in the section it belongs to, usually Standalone, with one line on when to reach for it.
6. Run `node scripts/check-marketplace.mjs` and `npx markdownlint-cli2`. Done when both report 0 errors.
7. Commit the skill, the README line, and any router edit together, subject `Add the <skill-name> skill to the <plugin> plugin`. Then invoke `bump-plugin-version` with a minor bump; it commits the manifests on their own, so the bump stays a separate commit.
